    ...
    <div className="ui-museum">
      ...
      <header className="ui-museum__hero">
        <div>
          ...
          <p>迁移自 UI-：风格博物馆、AI 风格融合、Project Genesis、PRD 规范和收藏库。</p>
        </div>
        <aside>
...
function buildStyleWorkbenchPrompt(item: UiStyleItem): string {
  ...
  return [
    ...
    `Web 规范：${item.web}`,
    `iOS 规范：${item.ios}`,
    `macOS 规范：${item.mac || 'macOS 版必须用 Toolbar、Sidebar/Split View、Inspector、键盘焦点和窗口状态承接桌面效率。'}`,
    `Android 规范：${item.android}`,
    `小程序规范：${item.mini}`,
...
function platformNotes(item: UiStyleItem): Record<PlatformPreviewKind, string> {
  return {
    web: item.web,
    ios: item.ios,
    mac: item.mac || 'macOS 版必须使用 Toolbar、Sidebar/Split View、Inspector、键盘焦点和窗口状态承接桌面效率。',
    android: item.android,
    mini: item.mini,
  }
}
...
function platformBlueprint(platform: PlatformPreviewKind) {
  const map: Record<PlatformPreviewKind, { title: string; anatomy: string[]; states: string[] }> = {
    ...
    mini: {
      ...
      states: ['menu sheet', 'tab switch', 'visible return', 'light Canvas fallback'],
    },
  }
  return map[platform]
}
...
function styleSpecificFamily(styleId: string | undefined, pattern: UiVisualTokens['pattern']): StyleRealization {
  ...
  const byStyle: Record<string, StyleRealization> = {
    ...
    jetsons: {
      ...
      title: 'Smart Dome',
      caption: 'Floating domes and friendly automation controls.',
      action: ['Auto', 'Hover', 'Relax'],
      rows: ['Dome panel', 'Robot task', 'Family state'],
    },
    'solarpunk-utopia': {
      ...base,
      kind: 'solarpunk',
  ...
  return styleId && byStyle[styleId] ? byStyle[styleId] : byPattern[pattern] || base
}
