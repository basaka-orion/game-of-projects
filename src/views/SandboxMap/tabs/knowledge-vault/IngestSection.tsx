...
import {
  ingestUrl, ingestPaste, isFileSupported, type IngestResult,
    ...
    for (let i = 0; i < supportedFiles.length; i++) {
      ...
      try {
        ...
        setBatchResults(prev => [...prev, {
          sourceId: '', pageId: '', drawerId: '', mode: 'fast' as const,
          pageTitle: fileName, triplesExtracted: 0, errors: [String(err)], fileName,
        }])
      }
    }
  ...
  return (
    <div className="kv-tab__ingest">
      <input ref={fileInputRef} type="file" multiple style={{ display: 'none' }}
        ...
        onChange={e => { const f = Array.from(e.target.files || []); if (f.length > 0) handleFilesDirectImport(f); e.target.value = '' }}
      />
      ...
      <div className="kv-tab__ingest-section">
        ...
        <div className="kv-tab__ingest-row">
          <input className="kv-tab__ingest-input" placeholder="https://..."
      ...
      <div className="kv-tab__ingest-section">
        ...
        <input className="kv-tab__ingest-input" placeholder="标题（可选）"
          value={pasteTitle} onChange={e => setPasteTitle(e.target.value)}
          style={{ marginBottom: 'var(--hd-space-xs)', display: 'block' }}
        />
        <textarea className="kv-tab__query-textarea" placeholder="粘贴文本内容..."
          value={pasteContent} onChange={e => setPasteContent(e.target.value)}
          style={{ minHeight: '100px', marginBottom: 'var(--hd-space-sm)' }}
        />
        <button className="kv-tab__btn" disabled={!pasteContent.trim() || isIngesting} onClick={handleIngestPaste}>摄入</button>
      </div>

      {/* 文件导入 */}
      <div className="kv-tab__ingest-section">
        <div className="kv-tab__ingest-label">📄 文件导入</div>
        <div className={`kv-tab__dropzone ${isDragOver ? 'kv-tab__dropzone--active' : ''}`}
        ...
        >
          ...
          <div style={{ fontSize: '0.7rem' }}>支持整个文件夹导入 · .md .txt .json .py .ts .js .go .rs .html .css .yaml .sql 等</div>
        </div>
      </div>
      ...
      <div className="kv-tab__ingest-section">
        ...
        <div className="kv-tab__ingest-row">
          <input className="kv-tab__ingest-input" placeholder="Vault 路径，如 /path/to/vault/Clippings"
            value={vaultPath} onChange={e => setVaultPath(e.target.value)}
          />
          <button className="kv-tab__btn" disabled={!vaultPath.trim() || isVaultImporting}
            onClick={async () => {
              setIsVaultImporting(true); setVaultResult(null)
              try {
              ...
              } catch (err) { setVaultResult({ imported: 0, skipped: 0, errors: [(err as Error).message] }) }
              finally { setIsVaultImporting(false) }
            }}
        ...
        {vaultResult && (
          <div style={{ fontSize: '0.75rem', color: 'var(--hd-text-secondary)', marginTop: 'var(--hd-space-xs)' }}>
            ...
            {vaultResult.errors.length > 0 && (<span style={{ color: 'var(--hd-error)' }}> · {vaultResult.errors.length} 个错误</span>)}
          </div>
        )}
      </div>
