    ...
    <div className="sandbox-map__tab" style={{ padding: 'var(--hd-space-lg)', overflowY: 'auto' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 'var(--hd-space-sm)' }}>
          ...
          <button className="sandbox-map__btn sandbox-map__btn--primary" onClick={() => setShowCreate(true)}>
            + 新建任务
          </button>
        </div>
      </div>
      ...
      {showLogs ? (
        <div>
          {logs.length === 0 ? (
            <div style={{ color: 'var(--hd-text-muted)', textAlign: 'center', padding: 40 }}>暂无执行日志</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--hd-space-xs)' }}>
              {logs.map((log) => (
                <div key={log.id} className="sandbox-map__memory-item" style={{ fontSize: '0.8rem' }}>
                  ...
                  {log.message && (
                    <span
                      style={{ color: 'var(--hd-error)', fontSize: '0.7rem', marginLeft: 8, maxWidth: 200 }}
                      title={log.message}
                    >
                ...
                <div key={task.id} className="sandbox-map__card" style={{ padding: 'var(--hd-space-md)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      {!task.enabled && (
                        <span
                          style={{ marginLeft: 8, fontSize: '0.7rem', color: 'var(--hd-text-muted)', opacity: 0.6 }}
                        >
                          (已禁用)
                        </span>
                      )}
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        ...
                        style={{
                          fontSize: '0.7rem',
                          color: task.enabled ? 'var(--hd-success)' : 'var(--hd-text-muted)',
                        }}
                  ...
                  <div style={{ marginTop: 6, fontSize: '0.75rem', color: 'var(--hd-text-muted)' }}>
                    Cron:{' '}
                    <code style={{ background: 'var(--hd-bg-hover)', padding: '1px 6px', borderRadius: 3 }}>
                      {task.cronExpression}
                    </code>
                    ...
                    {Array.isArray(task.platformTargets) &&
                      task.platformTargets.filter((p) => p.enabled).length > 0 && (
                        <span style={{ marginLeft: 8 }}>
                          {'| '}推送:{' '}
                          {task
                            .platformTargets!.filter((p) => p.enabled)
                            .map((p) => p.platform)
                            .join(', ')}
                        </span>
                      )}
