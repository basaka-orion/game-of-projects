...
const ZAI_LEGACY_BASE_URLS = new Set(['https://api.z.ai/api/paas/v4', 'https://api.z.ai/api/paas/v4/'])
...
export function resolveAgentConfig(agentRole: string): LLMConfig {
  // 1. 尝试读角色专属复杂模型配置；旧版 agent_{role}_* 作为兼容 fallback
  const heavyProvider = getSetting(`agent_${agentRole}_heavy_provider`, '')
  const legacyProvider = getSetting(`agent_${agentRole}_provider`, '')
  const agentProvider = heavyProvider || legacyProvider
  if (agentProvider) {
    // 该角色有独立配置
    const defaults = getDefaultConfig(agentProvider)
    const prefix = heavyProvider ? `agent_${agentRole}_heavy` : `agent_${agentRole}`
    return {
      provider: agentProvider as LLMConfig['provider'],
      apiKey: getSetting(`${prefix}_api_key`, ''),
      baseUrl: normalizeProviderBaseUrl(agentProvider, getSetting(`${prefix}_base_url`, defaults.baseUrl)),
  ...
  try {
    ...
    if (electronAPI?.sendToAI) {
      ...
      try {
        ...
        const result = await electronAPI.dbQuery('SELECT value FROM settings WHERE key = ?', ['llm_provider'])
    ...
    if (res.ok) {
      ...
      let apiMsg = ''
      try {
        apiMsg = JSON.parse(errText)?.error?.message || ''
      } catch {}
      if (res.status === 401) return { ok: false, message: '✗ API Key 无效或已过期' }
      if (res.status === 402 || apiMsg.includes('balance') || apiMsg.includes('Insufficient'))
...
async function openaiChatCompletion(
  ...
  const response = await fetch(`${config.baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      ...(shouldDisableThinking(config, maxTokens) ? { thinking: { type: 'disabled' } } : {}),
    }),
  })
