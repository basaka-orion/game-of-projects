...
export async function assembleContext(
  ...
  // Layer 5.5: 外脑主循环事件流（跨启蒙、画像、知识、推演、Agent 的最近状态）
  try {
    const events = await dbListOperatingEvents(8)
    if (events.length > 0) {
      ...
      const lines = events
        .slice(0, 8)
        .map((event) => `- [${stageLabel[event.stage] || event.stage}/${event.type}] ${event.title}: ${event.summary}`)
        .join('\n')
      layers.push({
        priority: 5,
        label: 'operating_loop_events',
        content: `## 外脑主循环最近事件：\n${lines}`,
        tokens: 0,
      })
    }
  ...
  if (projects.length > 0) {
    const lines = projects
      .slice(0, 5)
      .map((p) => `- 「${p.title}」存活率 ${p.survivalRate}% [${p.survivalGrade}] — ${p.oneLiner}`)
      .join('\n')
    layers.push({
      priority: 6,
    ...
    if (projects.length > 0) {
      ...
      if (warRoom) {
        ...
        const keywords = projects
          ...
          .flatMap((p) => p.title.split(/\s+/))
          .filter((k) => k.length > 1)
        const matched = warItems.filter((item) => keywords.some((kw) => item.content.includes(kw))).slice(0, 3)
  ...
  try {
    ...
    const hasDetailNeed = projects.some((p) =>
      detailSignals.some((s) => p.title.toLowerCase().includes(s) || p.oneLiner.toLowerCase().includes(s)),
    )
    ...
    if (hasDetailNeed) {
      ...
      if (keywords.length > 0) {
        ...
        if (drawerMatches.length > 0) {
          const drawerParts = drawerMatches
            .slice(0, 3)
            .map((d) => `- [Drawer:${d.id}] ${d.title}: ${d.rawContent.slice(0, 300)}`)
          layers.push({
            priority: 9,
            label: 'raw_drawers',
            content: `<raw-memory>\n${drawerParts.join('\n')}\n</raw-memory>`,
