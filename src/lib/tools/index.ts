...
import { validateCommand, type CommandValidation } from '../security/command-guard'
...
export async function executeTool(toolId: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!entry.definition.enabled) {
    return { success: false, data: null, error: `工具 ${toolId} 已禁用` }
  }

  // 参数验证
  for (const param of entry.definition.parameters) {
    if (param.required && (params[param.name] === undefined || params[param.name] === null)) {
      return { success: false, data: null, error: `缺少必需参数: ${param.name}` }
    }
  }

  const startedAt = Date.now()
  let commandValidation: CommandValidation | undefined
  let executionParams = params

  if (toolId === 'terminal') {
    commandValidation = validateCommand(String(params.command || ''))
    if (!commandValidation.allowed) {
      const result: ToolResult = {
        success: false,
        data: null,
        error: `终端命令被拒绝：${commandValidation.reason || '不符合安全策略'}`,
      }
    ...
    executionParams = {
      ...params,
      command: commandValidation.sanitized || String(params.command || ''),
    }
  }
  ...
  try {
    ...
    if (entry.executor) {
      result = await entry.executor(executionParams)
    } else {
      // 无本地执行器 → 尝试 MCP 调用
      const mcpServer = entry.definition.mcpServer || toolId
      result = await executeViaMCP(mcpServer, toolId, executionParams)
    }
...
export function parseToolCall(text: string): { tool: string; params: Record<string, unknown> } | null {
  const parseParams = (raw: string): Record<string, unknown> | null => {
    const trimmed = raw.trim()
    try {
      return JSON.parse(trimmed)
    } catch { /* tolerate partial model tool-call JSON */ }

    const params: Record<string, unknown> = {}
    const query = trimmed.match(/"query"\s*:\s*"([^"]+)"/)?.[1]
    if (query) params.query = query
    const maxResults = Number(trimmed.match(/"max_results"\s*:\s*(\d+)/)?.[1])
    if (Number.isFinite(maxResults) && maxResults > 0) params.max_results = maxResults
    if (query && params.max_results === undefined) params.max_results = 5
  ...
  if (jsonMatch) {
    const params = parseParams(jsonMatch[2])
    if (params) {
      return {
        tool: jsonMatch[1],
        params,
  ...
  // 尝试匹配 XML 标签格式；模型偶尔会漏掉结尾 `>`，这里做容错。
  const xmlMatch = text.match(/<tool_call\s+id=["']([^"']+)["']\s*>\s*([\s\S]*?)(?:<\/tool_call\s*>|<\/tool_call\b|$)/)
  if (xmlMatch) {
    const params = parseParams(xmlMatch[2])
    if (params) {
      return {
        tool: xmlMatch[1],
        params,
...
function initBuiltinTools(): void {
  ...
  registerTool({
    ...
    try {
      const result = await (window as any).electronAPI?.braveSearch?.(String(params.query), Number(params.max_results) || 5)
      if (result?.success && Array.isArray(result.data) && result.data.length > 0) {
        return { success: true, data: result.data }
      }
    } catch { /* Brave API 失败，继续后备 */ }

    // 后备：MCP Brave Search
    try {
      const result = await (window as any).electronAPI?.mcpCallTool?.('mcp-brave-search', 'brave_web_search', { query: params.query, count: params.max_results || 5 })
  ...
  registerTool({
    ...
    parameters: [
      ...
      { name: 'format', type: 'string', required: false, description: '输出格式 (markdown/text)', default: 'markdown' },
    ],
    mcpServer: 'mcp-fetch',
  }, async (params) => {
    const url = String(params.url || '').trim()
    if (!/^https?:\/\//i.test(url)) {
      return { success: false, data: null, error: '网页提取只允许 http/https URL' }
    }

    // 优先：通过主进程 fetch-url IPC 抓取，避免把 URL 拼进 shell 命令。
    try {
      const fetched = await (window as any).electronAPI?.fetchUrl?.(url)
      if (fetched?.content || fetched?.description || fetched?.title) {
        return {
          success: true,
          data: {
            title: fetched.title || '',
            url: fetched.url || url,
            content: String(fetched.content || fetched.description || '').slice(0, 8000),
            description: fetched.description || '',
            siteName: fetched.siteName || '',
          },
        }
      }
    } catch { /* fetch-url 失败，继续后备 */ }
    ...
    try {
      const ipcResult = await (window as any).electronAPI?.executeCommand?.(
        `curl -sL ${shellQuote(url)} -H "User-Agent: Mozilla/5.0" --max-time 15 | sed 's/<[^>]*>//g' | sed '/^$/d' | head -300`,
        20000
      )
    ...
    // 后备：MCP Fetch
    try {
      const result = await (window as any).electronAPI?.mcpCallTool?.('mcp-fetch', 'fetch', { url: params.url, format: params.format || 'markdown' })
      if (result) return { success: true, data: result }
    } catch { /* MCP 不可用 */ }
  ...
  registerTool({
    ...
    try {
      const result = await (window as any).electronAPI?.executeCommand?.(params.command, params.timeout)
      if (!result) {
        return { success: false, data: null, error: '终端执行失败: Electron API 不可用' }
      }
      if (typeof result === 'object') {
        const exitCode = Number(result.exitCode || 0)
        if (result.success === false || exitCode !== 0) {
          return {
            success: false,
            data: result,
            error: result.error || result.stderr || `终端命令退出码 ${exitCode}`,
          }
        }
      }
      return { success: true, data: result }
    } catch (err) {
      return { success: false, data: null, error: `终端执行失败: ${(err as Error).message}` }
    }
  })
  ...
  registerTool({
    ...
    try {
      ...
      if (!writer) {
        return { success: false, data: null, error: '文件写入失败: Electron API 不可用' }
      }
      const result = await writer(params.path, params.content)
      if (result?.success === false) {
        return { success: false, data: result, error: result.error || '文件写入失败' }
      }
  ...
  registerTool({
    ...
    parameters: [
      { name: 'query', type: 'string', required: true, description: '搜索关键词' },
      { name: 'sources', type: 'array', required: false, description: '搜索源 (palace/graph/agent)', default: ['palace', 'graph'] },
    ],
  }, async (params) => {
    ...
    try {
      ...
      const { memoryEntries, knowledgeTriples, synapsePaths } =
        await renderL3DeepSearchEnhanced(memory, params.query as string)
      results.push(...memoryEntries.map(e => ({ source: 'agent_memory', text: e.text })))
      results.push(...knowledgeTriples.map(t => ({
        source: 'knowledge_graph',
        text: `${t.subject} —[${t.predicate}]→ ${t.object} (${Math.round(t.confidence * 100)}%)`,
      })))
      results.push(...synapsePaths.map(p => ({
        source: 'synapse_path',
  ...
  registerTool({
    ...
    try {
      ...
      const desc = await chatCompletion(config, [
        { role: 'system', content: '你是一个图像描述生成器。根据用户的文字描述，生成一段详细的英文图像生成 prompt，用于 AI 绘画工具。' },
  ...
  registerTool({
    ...
    return { success: true, data: filtered.map(s => ({ id: s.id, name: s.name, enabled: s.enabled, source: s.source })) }
  })
  ...
  registerTool({
    ...
    if (action === 'create' && params.definition) {
      ...
      const skill = createSkill({ id: def.id as string, name: def.name as string, description: def.description as string })
      const skills = loadSkills()
      skills.push(skill)
  ...
  const playwrightExecutor = (toolName: string): ToolExecutor => async (params) => {
    try {
  ...
  registerTool({ id: 'browser_navigate', name: '浏览器导航', description: '导航到指定 URL', tier: 3, enabled: true,
    parameters: [{ name: 'url', type: 'string', required: true, description: '目标 URL' }, { name: 'wait', type: 'string', required: false, description: '等待条件' }],
  }, playwrightExecutor('browser_navigate'))

  registerTool({ id: 'browser_snapshot', name: '浏览器快照', description: '获取页面可访问性树快照', tier: 3, enabled: true,
    parameters: [],
  }, playwrightExecutor('browser_snapshot'))

  registerTool({ id: 'browser_click', name: '浏览器点击', description: '点击页面元素', tier: 3, enabled: true,
    parameters: [{ name: 'selector', type: 'string', required: true, description: '元素选择器' }, { name: 'button', type: 'string', required: false, description: '鼠标按钮', default: 'left' }],
  }, playwrightExecutor('browser_click'))

  registerTool({ id: 'browser_type', name: '浏览器输入', description: '在输入框中输入文本', tier: 3, enabled: true,
    parameters: [{ name: 'selector', type: 'string', required: true, description: '元素选择器' }, { name: 'text', type: 'string', required: true, description: '输入文本' }, { name: 'clear', type: 'boolean', required: false, description: '先清空', default: true }],
  }, playwrightExecutor('browser_type'))

  registerTool({ id: 'browser_scroll', name: '浏览器滚动', description: '滚动页面', tier: 3, enabled: true,
    parameters: [{ name: 'direction', type: 'string', required: true, description: '方向: up|down' }, { name: 'amount', type: 'number', required: false, description: '滚动量(px)', default: 300 }],
  }, playwrightExecutor('browser_scroll'))

  registerTool({ id: 'browser_back', name: '浏览器后退', description: '后退到上一页', tier: 3, enabled: true,
    parameters: [],
  }, playwrightExecutor('browser_back'))

  registerTool({ id: 'browser_press', name: '浏览器按键', description: '模拟键盘按键', tier: 3, enabled: true,
    parameters: [{ name: 'key', type: 'string', required: true, description: '按键（如 Enter, Tab）' }, { name: 'modifiers', type: 'array', required: false, description: '修饰键 (Control, Shift 等)' }],
  }, playwrightExecutor('browser_press'))

  registerTool({ id: 'browser_get_images', name: '浏览器图片', description: '获取页面所有图片', tier: 3, enabled: true,
    parameters: [{ name: 'selector', type: 'string', required: false, description: '过滤选择器' }, { name: 'min_width', type: 'number', required: false, description: '最小宽度', default: 100 }],
  }, playwrightExecutor('browser_get_images'))

  registerTool({ id: 'browser_vision', name: '浏览器视觉', description: '截图并用视觉 AI 分析', tier: 3, enabled: true,
    parameters: [{ name: 'prompt', type: 'string', required: false, description: '分析提示词' }, { name: 'selector', type: 'string', required: false, description: '截图区域选择器' }],
  }, playwrightExecutor('browser_vision'))

  registerTool({ id: 'browser_console', name: '浏览器控制台', description: '获取页面控制台输出', tier: 3, enabled: true,
    parameters: [{ name: 'level', type: 'string', required: false, description: '日志级别: log|warn|error|all', default: 'all' }, { name: 'limit', type: 'number', required: false, description: '最大条数', default: 50 }],
  ...
  registerTool({
    ...
    try {
      await run('CREATE TABLE IF NOT EXISTS todos (id TEXT PRIMARY KEY, content TEXT, priority TEXT DEFAULT \'medium\', status TEXT DEFAULT \'pending\', created_at TEXT DEFAULT(datetime("now","localtime")))')
      const action = params.action as string
      if (action === 'list') {
        const rows = await query<{ id: string; content: string; priority: string; status: string }>('SELECT id, content, priority, status FROM todos ORDER BY created_at DESC')
        return { success: true, data: rows }
      } else if (action === 'add') {
        const id = generateId()
        await run('INSERT INTO todos (id, content, priority) VALUES (?, ?, ?)', [id, params.content, params.priority || 'medium'])
  ...
  registerTool({
    ...
    try {
      ...
      const cmd = lang === 'python' ? `python3 -c ${JSON.stringify(params.code)}` : `${lang} -e ${JSON.stringify(params.code)}`
  ...
  registerTool({
    ...
    try {
      await run('CREATE TABLE IF NOT EXISTS cron_jobs (id TEXT PRIMARY KEY, schedule TEXT, task TEXT, enabled INTEGER DEFAULT 1, created_at TEXT DEFAULT(datetime("now","localtime")))')
      const action = params.action as string
      if (action === 'list') {
        const rows = await query<{ id: string; schedule: string; task: string; enabled: number }>('SELECT * FROM cron_jobs')
        return { success: true, data: rows }
  ...
  registerTool({ id: 'ha_list_entities', name: 'HA 实体列表', description: '列出 Home Assistant 实体', tier: 3, enabled: false,
    ...
    } catch (err) { return { success: false, data: null, error: (err as Error).message } }
  })

  registerTool({ id: 'ha_get_state', name: 'HA 状态查询', description: '获取 Home Assistant 实体状态', tier: 3, enabled: false,
  ...
  registerTool({ id: 'ha_list_services', name: 'HA 服务列表', description: '列出 Home Assistant 可用服务', tier: 3, enabled: false,
  ...
  registerTool({ id: 'ha_call_service', name: 'HA 服务调用', description: '调用 Home Assistant 服务控制设备', tier: 3, enabled: false,
    parameters: [
    ...
    try {
      ...
      const data = await haFetch(`/services/${domain}/${service}`, 'POST', { entity_id: params.entity_id, ...(params.data as object || {}) })
      return { success: true, data }
    } catch (err) { return { success: false, data: null, error: (err as Error).message } }
  })
}
