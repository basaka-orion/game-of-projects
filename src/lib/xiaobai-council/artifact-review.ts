...
function failureReasons(record: CouncilArtifactReviewRecord): string[] {
  ...
  return [
    ...
    baoyuScore >= PASSING_SCORE ? '' : `Baoyu/技术蓝图分项 ${baoyuScore}，低于 ${PASSING_SCORE}。`,
    ...
    record.baoyuChineseReadable ? '' : 'Baoyu/技术蓝图存在前后端/API/数据/部署边界不清或不可实施。',
    ...
    hasText(record.baoyuNotes, 8) ? '' : '缺少 Baoyu/技术蓝图可实施性的人工审稿摘要。',
